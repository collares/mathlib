...
variable [MeasurableSpace β] [Preorderₓ ι] {u v : ι → α → β} {f : Filtration ι m}
...
theorem add [Add β] [HasMeasurableAdd₂ β] (hu : Adapted f u) (hv : Adapted f v) : Adapted f (u + v) := fun i =>
  (hu i).add (hv i)

theorem neg [Neg β] [HasMeasurableNeg β] (hu : Adapted f u) : Adapted f (-u) := fun i => (hu i).neg

theorem smul [HasScalar ℝ β] [HasMeasurableSmul ℝ β] (c : ℝ) (hu : Adapted f u) : Adapted f (c • u) := fun i =>
  (hu i).const_smul c
...
def ProgMeasurable [MeasurableSpace ι] (f : Filtration ι m) (u : ι → α → β) : Prop :=
  ∀ i, measurable[Subtype.measurableSpace.Prod (f i)] fun p : Set.Iic i × α => u p.1 p.2

theorem prog_measurable_const [MeasurableSpace ι] (f : Filtration ι m) (b : β) :
    ProgMeasurable f (fun _ _ => b : ι → α → β) := fun i =>
  @measurable_const _ _ _ (Subtype.measurableSpace.Prod (f i)) _
...
protected theorem comp {t : ι → α → ι} (h : ProgMeasurable f u) (ht : ProgMeasurable f t) (ht_le : ∀ i x, t i x ≤ i) :
    ProgMeasurable f fun i x => u (t i x) x := by
  intro i
  have :
...
@[to_additive]
protected theorem mul [Mul β] [HasMeasurableMul₂ β] (hu : ProgMeasurable f u) (hv : ProgMeasurable f v) :
    ProgMeasurable f fun i x => u i x * v i x := fun i => (hu i).mul (hv i)

@[to_additive]
protected theorem finset_prod' {γ} [CommMonoidₓ β] [HasMeasurableMul₂ β] {U : γ → ι → α → β} {s : Finset γ}
...
@[to_additive]
protected theorem inv [Inv β] [HasMeasurableInv β] (hu : ProgMeasurable f u) : ProgMeasurable f fun i x => (u i x)⁻¹ :=
  fun i => (hu i).inv

@[to_additive]
protected theorem div [Div β] [HasMeasurableDiv₂ β] (hu : ProgMeasurable f u) (hv : ProgMeasurable f v) :
...
theorem prog_measurable_of_tendsto' {γ} [MeasurableSpace ι] [MetricSpace β] [BorelSpace β] (fltr : Filter γ)
    [fltr.ne_bot] [fltr.IsCountablyGenerated] {U : γ → ι → α → β} (h : ∀ l, ProgMeasurable f (U l))
    (h_tendsto : Tendsto U fltr (𝓝 u)) : ProgMeasurable f u := by
  refine' fun i =>
    @measurable_of_tendsto_metric' (Set.Iic i × α) β (MeasurableSpace.prod _ (f i)) _ _ _ γ _ _ fltr _ _
      (fun l => h l i) _
...
theorem prog_measurable_of_tendsto [MeasurableSpace ι] [MetricSpace β] [BorelSpace β] {U : ℕ → ι → α → β}
...
theorem Adapted.prog_measurable_of_continuous [EmetricSpace ι] [MeasurableSpace ι] [SecondCountableTopology ι]
    [OpensMeasurableSpace ι] [MetricSpace β] [BorelSpace β] (h : Adapted f u)
    (hu_cont : ∀ x, Continuous fun i => u i x) : ProgMeasurable f u := fun i =>
  @measurable_uncurry_of_continuous_of_measurable _ _ (Set.Iic i) _ _ _ _ _ _ _ _ _
    (fun x => (hu_cont x).comp continuous_induced_dom) fun j => (h j).mono (f.mono j.Prop) le_rfl
...
variable [MeasurableSpace ι] [TopologicalSpace ι] [OrderTopology ι] [SecondCountableTopology ι] [BorelSpace ι]
  [MeasurableSpace β] {u : ι → α → β} {τ : α → ι} {f : Filtration ι m}
...
theorem ProgMeasurable.measurable_stopped_process (hu : ProgMeasurable f u) (hτ : IsStoppingTime f τ) (i : ι) :
    Measurable (stoppedProcess u τ i) :=
...
theorem Adapted.prog_measurable_of_nat [MeasurableSpace β] [HasMeasurableAdd₂ β] (h : Adapted f u) :
  ...
  rw [this]
  refine' Finset.measurable_sum _ fun j hj => Measurable.ite _ _ _
  ...
  · exact @measurable_const _ (Set.Iic i × α) _ (MeasurableSpace.prod _ (f i)) _
    

/-- For filtrations indexed by `ℕ`, the stopped process obtained from an adapted process is
adapted. -/
theorem Adapted.stopped_process_of_nat [MeasurableSpace β] [HasMeasurableAdd₂ β] (hu : Adapted f u)
    (hτ : IsStoppingTime f τ) : Adapted f (stoppedProcess u τ) :=
  (hu.prog_measurable_of_nat.stoppedProcess hτ).Adapted

theorem Adapted.measurable_stopped_process_of_nat [MeasurableSpace β] [HasMeasurableAdd₂ β] (hτ : IsStoppingTime f τ)
    (hu : Adapted f u) (n : ℕ) : Measurable (stoppedProcess u τ n) :=
...
variable [MeasurableSpace β] [NormedGroup β] [HasMeasurableAdd₂ β] [BorelSpace β] {p : ℝ≥0∞} {μ : Measure α}
