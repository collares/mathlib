...
theorem measurable_uncurry_of_continuous_of_measurable {α β ι : Type _} [EmetricSpace ι] [MeasurableSpace ι]
    [SecondCountableTopology ι] [OpensMeasurableSpace ι] {mβ : MeasurableSpace β} [MetricSpace β] [BorelSpace β]
    {m : MeasurableSpace α} {u : ι → α → β} (hu_cont : ∀ x, Continuous fun i => u i x) (h : ∀ i, Measurable (u i)) :
    Measurable (Function.uncurry u) := by
  ...
  have h_meas : Measurable fun p : (t_sf n).range × α => u (↑p.fst) p.snd := by
    ...
    have : Encodable (t_sf n).range := Fintype.encodable ↥(t_sf n).range
