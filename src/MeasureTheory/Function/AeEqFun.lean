...
variable [Mul γ] [HasMeasurableMul₂ γ]
...
instance [AddMonoidₓ γ] [HasMeasurableAdd₂ γ] : AddMonoidₓ (α →ₘ[μ] γ) :=
  to_germ_injective.AddMonoid toGerm zero_to_germ add_to_germ fun _ _ => smul_to_germ _ _

instance [AddCommMonoidₓ γ] [HasMeasurableAdd₂ γ] : AddCommMonoidₓ (α →ₘ[μ] γ) :=
...
instance : Pow (α →ₘ[μ] γ) ℕ :=
  ⟨fun f n => comp (· ^ n) (Measurable.pow_const measurable_id _) f⟩

@[simp]
theorem mk_pow (f : α → γ) hf (n : ℕ) : (mk f hf : α →ₘ[μ] γ) ^ n = mk (f ^ n) (hf.pow_const n) :=
  rfl
...
@[to_additive]
instance [CommMonoidₓ γ] [HasMeasurableMul₂ γ] : CommMonoidₓ (α →ₘ[μ] γ) :=
  to_germ_injective.CommMonoid toGerm one_to_germ mul_to_germ pow_to_germ

section Inv

variable [Inv γ] [HasMeasurableInv γ]
...
variable [Div γ] [HasMeasurableDiv₂ γ]
...
instance [SubNegMonoidₓ γ] [HasMeasurableAdd₂ γ] [HasMeasurableNeg γ] : SubNegMonoidₓ (α →ₘ[μ] γ) :=
  to_germ_injective.SubNegMonoid toGerm zero_to_germ add_to_germ neg_to_germ sub_to_germ (fun _ _ => smul_to_germ _ _)
    fun _ _ => smul_to_germ _ _

instance [AddGroupₓ γ] [HasMeasurableAdd₂ γ] [HasMeasurableNeg γ] : AddGroupₓ (α →ₘ[μ] γ) :=
  to_germ_injective.AddGroup toGerm zero_to_germ add_to_germ neg_to_germ sub_to_germ (fun _ _ => smul_to_germ _ _)
    fun _ _ => smul_to_germ _ _

instance [AddCommGroupₓ γ] [HasMeasurableAdd₂ γ] [HasMeasurableNeg γ] : AddCommGroupₓ (α →ₘ[μ] γ) :=
...
variable [DivInvMonoidₓ γ] [HasMeasurableMul₂ γ] [HasMeasurableInv γ]

instance hasIntPow : Pow (α →ₘ[μ] γ) ℤ :=
  ⟨fun f n => comp (· ^ n) (measurable_id.pow_const _) f⟩

@[simp]
theorem mk_zpow (f : α → γ) hf (n : ℤ) : (mk f hf : α →ₘ[μ] γ) ^ n = mk (f ^ n) (hf.pow_const n) :=
  rfl
...
instance : DivInvMonoidₓ (α →ₘ[μ] γ) :=
  to_germ_injective.DivInvMonoid _ one_to_germ mul_to_germ inv_to_germ div_to_germ pow_to_germ zpow_to_germ

end DivInvMonoidₓ

@[to_additive]
instance [Groupₓ γ] [HasMeasurableMul₂ γ] [HasMeasurableDiv₂ γ] [HasMeasurableInv γ] : Groupₓ (α →ₘ[μ] γ) :=
