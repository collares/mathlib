...
open_locale Classical TopologicalSpace BigOperators Filter Ennreal Nnreal Interval
...
@[simp]
theorem mapₗ_apply [MeasurableSpace α] (f : α → β) (μ : Measure α) : mapₗ f μ = map f μ :=
  rfl

@[simp]
theorem map_add {m0 : MeasurableSpace α} (μ ν : Measure α) (f : α → β) : (μ + ν).map f = μ.map f + ν.map f :=
  (mapₗ f).map_add μ ν

@[simp]
theorem map_zero {m0 : MeasurableSpace α} (f : α → β) : (0 : Measure α).map f = 0 :=
  (mapₗ f).map_zero

@[simp]
theorem map_smul {m0 : MeasurableSpace α} (c : ℝ≥0∞) (μ : Measure α) (f : α → β) : (c • μ).map f = c • μ.map f :=
  (mapₗ f).map_smul c μ

/-- We can evaluate the pushforward on measurable sets. For non-measurable sets, see
  `measure_theory.measure.le_map_apply` and `measurable_equiv.map_apply`. -/
@[simp]
theorem map_apply {f : α → β} (hf : Measurable f) {s : Set β} (hs : MeasurableSet s) : μ.map f s = μ (f ⁻¹' s) := by
  rw [map, mapₗ]
  simp [dif_pos hf, hs]
...
theorem map_of_not_measurable {f : α → β} (hf : ¬Measurable f) : μ.map f = 0 := by
  rw [map, mapₗ, dif_neg hf, LinearMap.zero_apply]
...
@[mono]
theorem map_mono (f : α → β) (h : μ ≤ ν) : μ.map f ≤ ν.map f :=
...
theorem le_map_apply {f : α → β} (hf : Measurable f) (s : Set β) : μ (f ⁻¹' s) ≤ μ.map f s :=
  calc
    ...
    _ = μ.map f (ToMeasurable (μ.map f) s) := (map_apply hf <| measurable_set_to_measurable _ _).symm
    _ = μ.map f s := measure_to_measurable _
    

/-- Even if `s` is not measurable, `map f μ s = 0` implies that `μ (f ⁻¹' s) = 0`. -/
theorem preimage_null_of_map_null {f : α → β} (hf : Measurable f) {s : Set β} (hs : μ.map f s = 0) : μ (f ⁻¹' s) = 0 :=
  nonpos_iff_eq_zero.mp <| (le_map_apply hf s).trans_eq hs

theorem tendsto_ae_map {f : α → β} (hf : Measurable f) : Tendsto f μ.ae (μ.map f).ae := fun s hs =>
...
@[mono]
protected theorem map (h : μ ≪ ν) (f : α → β) : μ.map f ≪ ν.map f :=
  if hf : Measurable f then
...
theorem mem_ae_of_mem_ae_map {f : α → β} (hf : Measurable f) {s : Set β} (hs : s ∈ (μ.map f).ae) : f ⁻¹' s ∈ μ.ae :=
...
theorem ae_of_ae_map {f : α → β} (hf : Measurable f) {p : β → Prop} (h : ∀ᵐ y ∂μ.map f, p y) : ∀ᵐ x ∂μ, p (f x) :=
...
theorem ae_eq_comp' {ν : Measure β} {f : α → β} {g g' : β → δ} (hf : Measurable f) (h : g =ᵐ[ν] g') (h2 : μ.map f ≪ ν) :
    g ∘ f =ᵐ[μ] g' ∘ f :=
  (QuasiMeasurePreserving.mk hf h2).ae_eq h

theorem ae_eq_comp {f : α → β} {g g' : β → δ} (hf : Measurable f) (h : g =ᵐ[μ.map f] g') : g ∘ f =ᵐ[μ] g' ∘ f :=
...
theorem SigmaFinite.of_map (μ : Measure α) {f : α → β} (hf : Measurable f) (h : SigmaFinite (μ.map f)) :
    SigmaFinite μ :=
...
theorem comp_measurable [MeasurableSpace δ] {f : α → δ} {g : δ → β} (hg : AeMeasurable g (μ.map f))
...
theorem comp_measurable' {δ} [MeasurableSpace δ] {ν : Measureₓ δ} {f : α → δ} {g : δ → β} (hg : AeMeasurable g ν)
    (hf : Measurable f) (h : μ.map f ≪ ν) : AeMeasurable (g ∘ f) μ :=
...
theorem MeasurableEmbedding.ae_measurable_map_iff [MeasurableSpace γ] {f : α → β} (hf : MeasurableEmbedding f)
    {μ : Measureₓ α} {g : β → γ} : AeMeasurable g (μ.map f) ↔ AeMeasurable (g ∘ f) μ := by
