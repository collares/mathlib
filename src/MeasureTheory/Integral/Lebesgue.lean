...
@[simp]
theorem coe_smul [HasScalar K β] (c : K) (f : α →ₛ β) : ⇑(c • f) = c • f :=
  rfl
...
protected theorem induction {α γ} [MeasurableSpace α] [AddMonoidₓ γ] {P : SimpleFunc α γ → Prop}
    (h_ind : ∀ c {s} hs : MeasurableSet s, P (SimpleFunc.piecewise s hs (SimpleFunc.const _ c) (SimpleFunc.const _ 0)))
...
theorem lintegral_mono' {m : MeasurableSpace α} ⦃μ ν : Measure α⦄ (hμν : μ ≤ ν) ⦃f g : α → ℝ≥0∞⦄ (hfg : f ≤ g) :
  ...
  exact supr_le_supr fun φ => supr_le_supr2 fun hφ => ⟨le_transₓ hφ hfg, lintegral_mono (le_reflₓ φ) hμν⟩

theorem lintegral_mono ⦃f g : α → ℝ≥0∞⦄ (hfg : f ≤ g) : (∫⁻ a, f a ∂μ) ≤ ∫⁻ a, g a ∂μ :=
  lintegral_mono' (le_reflₓ μ) hfg

theorem lintegral_mono_nnreal {f g : α → ℝ≥0 } (h : f ≤ g) : (∫⁻ a, f a ∂μ) ≤ ∫⁻ a, g a ∂μ := by
...
theorem supr_lintegral_measurable_le_eq_lintegral (f : α → ℝ≥0∞) :
  ...
  · rw [lintegral]
    refine' bsupr_le fun i hi => le_supr_of_le i (le_supr_of_le i.Measurable (le_supr_of_le hi _))
...
theorem lintegral_eq_nnreal {m : MeasurableSpace α} (f : α → ℝ≥0∞) (μ : Measure α) :
  ...
  rw [lintegral]
  refine' le_antisymmₓ (bsupr_le fun φ hφ => _) (supr_le_supr2 fun φ => ⟨φ.map (coe : ℝ≥0 → ℝ≥0∞), le_rfl⟩)
...
theorem exists_simple_func_forall_lintegral_sub_lt_of_pos {f : α → ℝ≥0∞} (h : (∫⁻ x, f x ∂μ) ≠ ∞) {ε : ℝ≥0∞}
  ...
  have : (map coe φ).lintegral μ ≠ ∞ := ne_top_of_le_ne_top h (le_bsupr φ hle)
...
theorem supr2_lintegral_le {ι : Sort _} {ι' : ι → Sort _} (f : ∀ i, ι' i → α → ℝ≥0∞) :
    (⨆ (i) (h : ι' i), ∫⁻ a, f i h a ∂μ) ≤ ∫⁻ a, ⨆ (i) (h : ι' i), f i h a ∂μ := by
...
theorem lintegral_add' {f g : α → ℝ≥0∞} (hf : AeMeasurable f μ) (hg : AeMeasurable g μ) :
    (∫⁻ a, f a + g a ∂μ) = (∫⁻ a, f a ∂μ) + ∫⁻ a, g a ∂μ :=
  calc
    (∫⁻ a, f a + g a ∂μ) = ∫⁻ a, hf.mk f a + hg.mk g a ∂μ :=
      lintegral_congr_ae (EventuallyEq.add hf.ae_eq_mk hg.ae_eq_mk)
...
theorem lintegral_zero_fun : (∫⁻ a : α, (0 : α → ℝ≥0∞) a ∂μ) = 0 := by
  simp
...
theorem mul_meas_ge_le_lintegral {f : α → ℝ≥0∞} (hf : Measurable f) (ε : ℝ≥0∞) : ε * μ { x | ε ≤ f x } ≤ ∫⁻ a, f a ∂μ :=
  by
  have : MeasurableSet { a : α | ε ≤ f a } := hf measurable_set_Ici
...
@[simp]
theorem lintegral_eq_zero_iff {f : α → ℝ≥0∞} (hf : Measurable f) : (∫⁻ a, f a ∂μ) = 0 ↔ f =ᵐ[μ] 0 := by
  refine' Iff.intro (fun h => _) fun h => _
  · have : ∀ n : ℕ, ∀ᵐ a ∂μ, f a < n⁻¹ := by
      intro n
    ...
    exact (lt_irreflₓ _ <| lt_transₓ hn <| ha n).elim
    
  · calc (∫⁻ a, f a ∂μ) = ∫⁻ a, 0 ∂μ := lintegral_congr_ae h _ = 0 := lintegral_zero
...
theorem lintegral_strict_mono_of_ae_le_of_ae_lt_on {f g : α → ℝ≥0∞} (hf : Measurable f) (hg : Measurable g)
    (hfi : (∫⁻ x, f x ∂μ) ≠ ∞) (h_le : f ≤ᵐ[μ] g) {s : Set α} (hμs : μ s ≠ 0) (h : ∀ᵐ x ∂μ, x ∈ s → f x < g x) :
    (∫⁻ x, f x ∂μ) < ∫⁻ x, g x ∂μ := by
  rw [← tsub_pos_iff_lt, ← lintegral_sub hg hf hfi h_le]
  by_contra hnlt
  rw [not_ltₓ, nonpos_iff_eq_zero, lintegral_eq_zero_iff (hg.sub hf), Filter.EventuallyEq] at hnlt
  simp only [ae_iff, tsub_eq_zero_iff_le, Pi.zero_apply, not_ltₓ, not_leₓ] at hnlt h
  refine' hμs _
  push_neg  at h
  have hs_eq : s = { a : α | a ∈ s ∧ g a ≤ f a } ∪ { a : α | a ∈ s ∧ f a < g a } := by
    ext1 x
    simp_rw [Set.mem_union, Set.mem_set_of_eq, ← not_leₓ]
    tauto
  rw [hs_eq]
...
theorem set_lintegral_strict_mono {f g : α → ℝ≥0∞} {s : Set α} (hsm : MeasurableSet s) (hs : μ s ≠ 0)
    (hf : Measurable f) (hg : Measurable g) (hfi : (∫⁻ x in s, f x ∂μ) ≠ ∞) (h : ∀ᵐ x ∂μ, x ∈ s → f x < g x) :
      ...
      calc
        ((∫⁻ a, f 0 a ∂μ) - ∫⁻ a, ⨅ n, f n a ∂μ) = ∫⁻ a, f 0 a - ⨅ n, f n a ∂μ :=
          (lintegral_sub (h_meas 0) (measurable_infi h_meas)
              (ne_top_of_le_ne_top h_fin <| lintegral_mono fun a => infi_le _ _)
        ...
        _ = ⨆ n, (∫⁻ a, f 0 a ∂μ) - ∫⁻ a, f n a ∂μ :=
          ...
          congr_argₓ supr <|
            funext fun n =>
              lintegral_sub (h_meas _) (h_meas _) (ne_top_of_le_ne_top h_fin <| lintegral_mono_ae <| h_mono n)
                (h_mono n)
...
theorem lintegral_liminf_le' {f : ℕ → α → ℝ≥0∞} (h_meas : ∀ n, AeMeasurable (f n) μ) :
  ...
  calc
    ...
    _ ≤ ⨆ n : ℕ, ⨅ i ≥ n, ∫⁻ a, f i a ∂μ := supr_le_supr fun n => le_infi2_lintegral _
...
theorem limsup_lintegral_le {f : ℕ → α → ℝ≥0∞} {g : α → ℝ≥0∞} (hf_meas : ∀ n, Measurable (f n))
  ...
  calc
    ...
    _ ≤ ⨅ n : ℕ, ∫⁻ a, ⨆ i ≥ n, f i a ∂μ := infi_le_infi fun n => supr2_lintegral_le _
...
theorem lintegral_Union [Encodable β] {s : β → Set α} (hm : ∀ i, MeasurableSet (s i)) (hd : Pairwise (Disjoint on s))
    (f : α → ℝ≥0∞) : (∫⁻ a in ⋃ i, s i, f a ∂μ) = ∑' i, ∫⁻ a in s i, f a ∂μ := by
...
theorem lintegral_union {f : α → ℝ≥0∞} {A B : Set α} (hA : MeasurableSet A) (hB : MeasurableSet B)
    (hAB : Disjoint A B) : (∫⁻ a in A ∪ B, f a ∂μ) = (∫⁻ a in A, f a ∂μ) + ∫⁻ a in B, f a ∂μ := by
...
theorem lintegral_map_le {mβ : MeasurableSpace β} (f : β → ℝ≥0∞) {g : α → β} (hg : Measurable g) :
  ...
  refine' bsupr_le fun i hi => supr_le fun h'i => _
...
theorem lintegral_encodable {α : Type _} {m : MeasurableSpace α} [Encodable α] [MeasurableSingletonClass α]
    (f : α → ℝ≥0∞) (μ : Measure α) : (∫⁻ a, f a ∂μ) = ∑' a, f a * μ {a} := by
...
theorem ae_lt_top' {f : α → ℝ≥0∞} (hf : AeMeasurable f μ) (h2f : (∫⁻ x, f x ∂μ) ≠ ∞) : ∀ᵐ x ∂μ, f x < ∞ :=
...
theorem lintegral_with_density_le_lintegral_mul (μ : Measure α) {f : α → ℝ≥0∞} (f_meas : Measurable f) (g : α → ℝ≥0∞) :
  ...
  rw [← supr_lintegral_measurable_le_eq_lintegral, ← supr_lintegral_measurable_le_eq_lintegral]
  refine' bsupr_le fun i i_meas => supr_le fun hi => _
...
theorem lintegral_trim {μ : Measure α} (hm : m ≤ m0) {f : α → ℝ≥0∞} (hf : @Measurable _ _ m _ f) :
...
theorem univ_le_of_forall_fin_meas_le {μ : Measure α} (hm : m ≤ m0) [@SigmaFinite _ m (μ.trim hm)] (C : ℝ≥0∞)
...
theorem lintegral_le_of_forall_fin_meas_le_of_measurable {μ : Measure α} (hm : m ≤ m0) [@SigmaFinite _ m (μ.trim hm)]
...
theorem lintegral_le_of_forall_fin_meas_le' {μ : Measure α} (hm : m ≤ m0) [@SigmaFinite _ m (μ.trim hm)] (C : ℝ≥0∞)
