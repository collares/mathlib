...
theorem is_iso_of_bijective {x y : SimplexCategory.{u}} {f : x ⟶ y} (hf : Function.Bijective f.toOrderHom.toFun) :
    IsIso f :=
...
@[simp]
def orderIsoOfIso {x y : SimplexCategory.{u}} (e : x ≅ y) : Finₓ (x.len + 1) ≃o Finₓ (y.len + 1) :=
  Equivₓ.toOrderIso
...
theorem iso_eq_iso_refl {x : SimplexCategory.{u}} (e : x ≅ x) : e = Iso.refl x := by
...
theorem eq_id_of_is_iso {x : SimplexCategory.{u}} {f : x ⟶ x} (hf : IsIso f) : f = 𝟙 _ :=
...
theorem eq_comp_δ_of_not_surjective' {n : ℕ} {Δ : SimplexCategory} (θ : Δ ⟶ mk (n + 1)) (i : Finₓ (n + 2))
  ...
  · have h' := le_antisymmₓ (Finₓ.le_last i) (not_lt.mp h)
    subst h'
    use θ ≫ σ (Finₓ.last _)
    ext1
    ext1
...
theorem eq_id_of_mono {x : SimplexCategory.{u}} (i : x ⟶ x) [Mono i] : i = 𝟙 _ := by
...
theorem eq_id_of_epi {x : SimplexCategory.{u}} (i : x ⟶ x) [Epi i] : i = 𝟙 _ := by
  apply eq_id_of_is_iso
  apply is_iso_of_bijective
