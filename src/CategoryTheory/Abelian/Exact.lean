...
instance Exact.op [Exact f g] : Exact g.op f.op := by
  rw [exact_iff]
  refine'
    ⟨by
      simp [← op_comp], _⟩
  ...
  simp only [unop_comp, cokernel.π_op, eq_to_hom_refl, kernel.ι_op, category.id_comp, category.assoc,
    kernel_comp_cokernel_assoc, zero_comp, comp_zero, unop_zero]
...
theorem Exact.op_iff : Exact g.op f.op ↔ Exact f g :=
  ⟨fun e => by
    ...
    dsimp
    skip
    infer_instance, fun e => @Exact.op _ _ _ _ e⟩

instance Exact.unop {X Y Z : Cᵒᵖ} (g : X ⟶ Y) (f : Y ⟶ Z) [h : Exact g f] : Exact f.unop g.unop := by
