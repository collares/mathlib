...
@[simp]
theorem Fork.IsLimit.lift_of_ι_ι {s : Fork f g} (hs : IsLimit s) {W : C} {k : W ⟶ X} (hk : k ≫ f = k ≫ g) :
    hs.lift (Fork.ofι k hk) ≫ s.ι = k :=
  hs.fac _ _

@[simp]
theorem Cofork.IsColimit.π_desc_of_π {s : Cofork f g} (hs : IsColimit s) {W : C} {k : Y ⟶ W} (hk : f ≫ k = g ≫ k) :
    s.π ≫ hs.desc (Cofork.ofπ k hk) = k :=
  hs.fac _ _
...
def splitMonoOfIdempotentOfIsLimitFork {X : C} {f : X ⟶ X} (hf : f ≫ f = f) {c : Fork f (𝟙 X)} (i : IsLimit c) :
    SplitMono c.ι where
  retraction :=
    i.lift
      (Fork.ofι f
        (by
          simp [hf]))
  id' := by
    let this' := mono_of_is_limit_parallel_pair i
...
def splitMonoOfIdempotentEqualizer {X : C} {f : X ⟶ X} (hf : f ≫ f = f) [HasEqualizer f (𝟙 X)] :
    SplitMono (equalizer.ι f (𝟙 X)) :=
...
def splitEpiOfIdempotentOfIsColimitCofork {X : C} {f : X ⟶ X} (hf : f ≫ f = f) {c : Cofork f (𝟙 X)} (i : IsColimit c) :
    SplitEpi c.π where
  section_ :=
    i.desc
      (Cofork.ofπ f
        (by
          simp [hf]))
  id' := by
    let this' := epi_of_is_colimit_parallel_pair i
...
def splitEpiOfIdempotentCoequalizer {X : C} {f : X ⟶ X} (hf : f ≫ f = f) [HasCoequalizer f (𝟙 X)] :
    SplitEpi (coequalizer.π f (𝟙 X)) :=
