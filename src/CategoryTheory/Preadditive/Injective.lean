...
```
-/
def Exact.desc {J Q R S : C} [Injective J] (h : R ⟶ J) (f : Q ⟶ R) (g : R ⟶ S) [Exact g.op f.op] (w : f ≫ h = 0) :
    S ⟶ J :=
  (Exact.lift h.op g.op f.op (congr_argₓ Quiver.Hom.op w)).unop

@[simp]
theorem Exact.comp_desc {J Q R S : C} [Injective J] (h : R ⟶ J) (f : Q ⟶ R) (g : R ⟶ S) [Exact g.op f.op]
    (w : f ≫ h = 0) : g ≫ Exact.desc h f g w = h := by
  convert congr_argₓ Quiver.Hom.unop (exact.lift_comp h.op g.op f.op (congr_argₓ Quiver.Hom.op w))
