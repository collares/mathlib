...
def comap (f : M →* N) (S : Submonoid N) : Submonoid M where
  Carrier := f ⁻¹' S
  one_mem' :=
    show f 1 ∈ S by
...
def map (f : M →* N) (S : Submonoid M) : Submonoid N where
  Carrier := f '' S
...
@[simp, to_additive]
theorem mem_map {f : M →* N} {S : Submonoid M} {y : N} : y ∈ S.map f ↔ ∃ x ∈ S, f x = y :=
  mem_image_iff_bex
...
@[to_additive]
theorem gc_map_comap (f : M →* N) : GaloisConnection (map f) (comap f) := fun S T => map_le_iff_le_comap
...
@[to_additive]
theorem pow_mem {M : Type _} [Monoidₓ M] (S : Submonoid M) {x : M} (hx : x ∈ S) (n : ℕ) : x ^ n ∈ S := by
  induction' n with n ih
  · simpa only [pow_zeroₓ] using S.one_mem
    
  · simpa only [pow_succₓ] using S.mul_mem hx ih
    

instance _root_.add_submonoid.has_nsmul {M : Type _} [AddMonoidₓ M] (S : AddSubmonoid M) : HasScalar ℕ S :=
  ⟨fun n x => ⟨n • x, S.nsmul_mem x.Prop _⟩⟩

@[to_additive]
instance hasPow {M : Type _} [Monoidₓ M] (S : Submonoid M) : Pow S ℕ :=
  ⟨fun x n => ⟨x ^ n, S.pow_mem x.Prop _⟩⟩
...
@[to_additive]
theorem mrange_top_iff_surjective {N} [MulOneClassₓ N] {f : M →* N} :
...
theorem mrange_top_of_surjective {N} [MulOneClassₓ N] (f : M →* N) (hf : Function.Surjective f) :
...
@[to_additive]
instance decidableMemMker [DecidableEq N] (f : M →* N) : DecidablePred (· ∈ f.mker) := fun x =>
  decidableOfIff (f x = 1) f.mem_mker
...
instance [MulAction M' α] [HasFaithfulScalar M' α] (S : Submonoid M') : HasFaithfulScalar S α where
