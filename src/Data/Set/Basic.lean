...
instance {α : Type _} : BooleanAlgebra (Set α) :=
  { (inferInstance : BooleanAlgebra (α → Prop)) with sup := (· ∪ ·), le := (· ≤ ·), lt := (· < ·), inf := (· ∩ ·),
    bot := ∅, Compl := Set.Compl, top := Univ, sdiff := (· \ ·) }
...
instance SetCoe.canLift (s : Set α) : CanLift α s where
  coe := coe
  cond := fun a => a ∈ s
...
variable {α : Type u} {β : Type v} {γ : Type w} {ι : Sort x} {a b : α} {s t : Set α}
...
theorem insert_inter_of_mem {s₁ s₂ : Set α} {a : α} (h : a ∈ s₂) : insert a s₁ ∩ s₂ = insert a (s₁ ∩ s₂) := by
  simp [Set.insert_inter, h]

theorem insert_inter_of_not_mem {s₁ s₂ : Set α} {a : α} (h : a ∉ s₂) : insert a s₁ ∩ s₂ = s₁ ∩ s₂ := by
  ext x
...
theorem image_compl_subset {f : α → β} {s : Set α} (H : Injective f) : f '' sᶜ ⊆ (f '' s)ᶜ :=
  subset_compl_iff_disjoint.2 <| by
    simp [image_inter H]
...
theorem Nonempty.image2 (hs : s.Nonempty) (ht : t.Nonempty) : (Image2 f s t).Nonempty := by
  cases' hs with a ha
  cases' ht with b hb
