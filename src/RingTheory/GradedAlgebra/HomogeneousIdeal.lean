...
instance : HasInf (HomogeneousIdeal 𝒜) where
  inf := fun I J => ⟨I.toIdeal⊓J.toIdeal, I.IsHomogeneous.inf J.IsHomogeneous⟩
...
instance : HasInfₓ (HomogeneousIdeal 𝒜) :=
  ⟨fun ℐ => ⟨inf (to_ideal '' ℐ), Ideal.IsHomogeneous.Inf fun _ ⟨I, _, hI⟩ => hI ▸ I.IsHomogeneous⟩⟩

@[simp]
theorem to_ideal_Inf (ℐ : Set (HomogeneousIdeal 𝒜)) : (inf ℐ).toIdeal = inf (to_ideal '' ℐ) :=
  rfl

@[simp]
theorem to_ideal_infi {ι' : Sort _} (s : ι' → HomogeneousIdeal 𝒜) : (⨅ i, s i).toIdeal = ⨅ i, (s i).toIdeal := by
  rw [infi, infi, to_ideal_Inf, ← Set.range_comp]

instance : HasSup (HomogeneousIdeal 𝒜) where
  sup := fun I J => ⟨I.toIdeal⊔J.toIdeal, I.IsHomogeneous.sup J.IsHomogeneous⟩
...
instance : HasSupₓ (HomogeneousIdeal 𝒜) :=
  ⟨fun ℐ => ⟨sup (to_ideal '' ℐ), Ideal.IsHomogeneous.Sup fun _ ⟨I, _, hI⟩ => hI ▸ I.IsHomogeneous⟩⟩

@[simp]
theorem to_ideal_Sup (ℐ : Set (HomogeneousIdeal 𝒜)) : (sup ℐ).toIdeal = (sup (to_ideal '' ℐ) : Ideal A) :=
  rfl

@[simp]
theorem coe_supr {ι' : Sort _} (s : ι' → HomogeneousIdeal 𝒜) : (⨆ i, s i).toIdeal = ⨆ i, (s i).toIdeal := by
  rw [supr, supr, to_ideal_Sup, ← Set.range_comp]
        ...
        ⟨Ideal.span (GradedAlgebra.proj 𝒜 i '' I),
          Ideal.is_homogeneous_span 𝒜 _
            (by
              ...
              apply SetLike.is_homogeneous_coe)⟩ :=
  by
  ext1 <;> rw [Ideal.to_ideal_homogeneous_hull_eq_supr, coe_supr] <;> rfl
