...
open GradedAlgebra SetLike Finset
...
theorem Ideal.IsHomogeneous.is_prime_of_homogeneous_mem_or_mem {I : Ideal A} (hI : I.IsHomogeneous 𝒜) (I_ne_top : I ≠ ⊤)
  ...
  ⟨I_ne_top, by
    ...
    have mem_I : proj 𝒜 max₁ x * proj 𝒜 max₂ y ∈ I := by
      set antidiag := ((support 𝒜 x).product (support 𝒜 y)).filter fun z : ι × ι => z.1 + z.2 = max₁ + max₂ with ha
...
theorem Ideal.IsHomogeneous.radical {I : Ideal A} (h : I.IsHomogeneous 𝒜) : I.radical.IsHomogeneous 𝒜 := by
  convert (Inf { J : HomogeneousIdeal 𝒜 | I ≤ J.toIdeal ∧ J.to_ideal.is_prime }).IsHomogeneous using 2
  rw [h.radical_eq, HomogeneousIdeal.to_ideal_Inf]
  congr 1
  ext1
  rw [Set.mem_image, Set.mem_set_of_eq]
  exact
    ⟨fun h => ⟨⟨x, h.1⟩, h.2, rfl⟩, by
