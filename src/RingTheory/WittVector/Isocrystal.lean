...
def FractionRing.frobenius : K(p,k) ≃+* K(p,k) :=
  IsFractionRing.fieldEquivOfRingEquiv (RingEquiv.ofBijective _ (WittVector.frobenius_bijective p k))
...
-- ././Mathport/Syntax/Translate/Basic.lean:980:9: unsupported derive handler module «exprK( , )»(p, k)
...
theorem isocrystal_classification (k : Type _) [Field k] [IsAlgClosed k] [CharP k p] (V : Type _) [AddCommGroupₓ V]
  ...
  obtain ⟨b, hb, m, hmb : φ(p,k) b * a = p ^ m * b⟩ := WittVector.exists_frobenius_solution_fraction_ring p ha
  use m
