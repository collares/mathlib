...
theorem adjoin_induction₂ {p : A → A → Prop} {a b : A} (ha : a ∈ adjoin R s) (hb : b ∈ adjoin R s)
    ...
    (Hadd_alg : ∀ x y r, p x (algebraMap R A r) → p y (algebraMap R A r) → p (x + y) (algebraMap R A r))
    (Hmul_alg : ∀ x y r, p x (algebraMap R A r) → p y (algebraMap R A r) → p (x * y) (algebraMap R A r)) : p a b := by
  ...
  · exact adjoin_induction ha (Halg_right r) (fun r' => Halg r' r) (fun x y => Hadd_alg x y r) fun x y => Hmul_alg x y r
    ...
    mul_comm := fun x y => by
      ...
      exact
        adjoin_induction₂ x.prop y.prop hcomm
          ...
          (fun _ _ _ _ _ => by
            simp only [add_mulₓ, mul_addₓ, commutes])
          fun _ _ _ _ _ => by
          rw [commutes] }
