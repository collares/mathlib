...
@[simp, to_additive]
theorem coe_one [One α] : ↑(1 : Finset α) = (1 : Set α) :=
  coe_singleton 1
...
theorem coe_mul (s t : Finset α) : (↑(s * t) : Set α) = ↑s * ↑t := by
  ext
...
protected def hasNsmul [DecidableEq α] [AddMonoidₓ α] : HasScalar ℕ (Finset α) where
  smul := fun n s => nsmulRec n s

/-- Repeated pointwise multiplication (not the same as pointwise repeated multiplication!) of a
`finset`. -/
@[to_additive]
protected def hasNpow [DecidableEq α] [Monoidₓ α] : Pow (Finset α) ℕ where
  pow := fun s n => npowRec n s

localized [Pointwise] attribute [instance] Finset.hasNsmul Finset.hasNpow

@[simp, to_additive]
theorem coe_pow [DecidableEq α] [Monoidₓ α] (s : Finset α) (n : ℕ) : ↑(s ^ n) = (s ^ n : Set α) := by
...
@[to_additive
      "If a finset `U` contained in the product of two sets `S * S'`, we can find two\nfinsets `T`, `T'` such that `U ⊆ T * T'` and `T * T' ⊆ S * S'`."]
