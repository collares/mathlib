...
structure BasicSmoothVectorBundleCore {𝕜 : Type _} [NondiscreteNormedField 𝕜] {E : Type _} [NormedGroup E]
  ...
  coordChange : Atlas H M → Atlas H M → H → F →ₗ[𝕜] F
...
def tangentBundleCore : BasicSmoothVectorBundleCore I M E where
  ...
  coord_change_comp := fun i j u x hx => by
    ...
    simp only [forall_const, eq_self_iff_true, LinearMap.coe_comp, ContinuousLinearMap.coe_comp]
...
def TangentSpace (x : M) : Type _ :=
  (tangentBundleCore I M).toTopologicalVectorBundleCore.Fiber x
...
def TangentBundle :=
  (tangentBundleCore I M).toTopologicalVectorBundleCore.TotalSpace
...
local notation "TM" => Bundle.TotalSpace (TangentSpace I : M → Type _)
...
instance : TopologicalVectorBundle 𝕜 E (TangentSpace I : M → Type _) := by
  infer_instance

instance topologicalSpaceTotalTangentBundle : TopologicalSpace TM := by
  infer_instance

instance : ChartedSpace (ModelProd H E) TM := by
  infer_instance
...
theorem tangent_bundle_model_space_chart_at (p : TangentBundle I H) :
  ...
  · rintro ⟨x_fst, x_snd⟩
    simp' only [BasicSmoothVectorBundleCore.toTopologicalVectorBundleCore, tangentBundleCore, A,
      ContinuousLinearMap.coe_id', BasicSmoothVectorBundleCore.chart, chart_at, ContinuousLinearMap.coe_coe] with
      mfld_simps
