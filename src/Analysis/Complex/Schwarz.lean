...
theorem schwarz_aux {f : ℂ → ℂ} (hd : DifferentiableOn ℂ f (Ball c R₁)) (h_maps : MapsTo f (Ball c R₁) (Ball (f c) R₂))
  ...
  replace hd : DifferentiableOn ℂ (dslope f c) (closed_ball c r) :=
    ((differentiable_on_dslope <| ball_mem_nhds _ hR₁).mpr hd).mono (closed_ball_subset_ball hr.2)
  refine'
    norm_le_of_forall_mem_frontier_norm_le (is_compact_closed_ball c r) hd.continuous_on (hd.mono interior_subset) _
      hr.1.le
