...
theorem algebra_map_exp_comm_of_mem_ball [CompleteSpace 𝕂] (x : 𝕂)
  ...
  by
  convert (algebraMapClm 𝕂 𝔸).map_tsum (exp_series_field_summable_of_mem_ball x hx)
  · exact congr_funₓ exp_eq_tsum_field x
    
  · convert congr_funₓ (exp_eq_tsum : exp 𝕂 𝔸 = _) (algebraMap 𝕂 𝔸 x)
    simp_rw [← map_pow, ← algebra_map_clm_coe, ← (algebraMapClm 𝕂 𝔸).map_smul, smul_eq_mul, mul_comm, ←
      div_eq_mul_one_div]
