...
theorem cof_eq_Inf_lsub (o : Ordinal.{u}) :
  ...
  · rcases cof_eq (· < ·) with ⟨S, hS, hS'⟩
    ...
    refine'
      ⟨S, f, le_antisymmₓ (lsub_le.2 fun i => typein_lt_self i) (le_of_forall_lt fun a ha => _), by
        rwa [type_lt o] at hS'⟩
    rw [← type_lt o] at ha
...
theorem is_regular_aleph_one : IsRegular (aleph 1) := by
  rw [← succ_omega]
  exact succ_is_regular le_rfl

theorem aleph'_succ_is_regular {o : Ordinal} (h : Ordinal.omega ≤ o) : IsRegular (aleph' o.succ) := by
  rw [aleph'_succ]
  exact succ_is_regular (omega_le_aleph'.2 h)

theorem aleph_succ_is_regular {o : Ordinal} : IsRegular (aleph o.succ) := by
  rw [aleph_succ]
  exact succ_is_regular (omega_le_aleph o)
