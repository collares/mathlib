...
-- ././Mathport/Syntax/Translate/Basic.lean:946:4: unsupported set replacement {(«expr / »(x, y)) | (x «expr ∈ » subring.closure s) (y «expr ∈ » subring.closure s)}
...
theorem mem_closure {x : K} {s : Set K} : x ∈ closure s ↔ ∀ S : Subfield K, s ⊆ S → x ∈ S :=
  ⟨fun t le => x_eq ▸ t.div_mem (Subring.mem_closure.mp hy t.toSubring le) (Subring.mem_closure.mp hz t.toSubring le),
...
def codRestrictField (f : K →+* L) (s : Subfield L) (h : ∀ x, f x ∈ s) : K →+* s where
  toFun := fun x => ⟨f x, h x⟩
...
def restrictField (f : K →+* L) (s : Subfield K) : s →+* L :=
  f.comp s.Subtype
