...
protected irreducible_def smul [HasScalar R (FractionRing K[X])] : R → Ratfunc K → Ratfunc K
  | r, ⟨p⟩ => ⟨r • p⟩

instance [HasScalar R (FractionRing K[X])] : HasScalar R (Ratfunc K) :=
  ⟨Ratfunc.smul⟩

theorem of_fraction_ring_smul [HasScalar R (FractionRing K[X])] (c : R) (p : FractionRing K[X]) :
    of_fraction_ring (c • p) = c • of_fraction_ring p := by
  unfold HasScalar.smul Ratfunc.smul

theorem to_fraction_ring_smul [HasScalar R (FractionRing K[X])] (c : R) (p : Ratfunc K) :
...
@[simp]
theorem num_div (p : K[X]) {q : K[X]} (hq : q ≠ 0) :
...
theorem num_one : num (1 : Ratfunc K) = 1 := by
  convert num_div (1 : K[X]) one_ne_zero <;> simp
...
theorem int_degree_add_le {x y : Ratfunc K} (hx : x ≠ 0) (hy : y ≠ 0) (hxy : x + y ≠ 0) :
