...
theorem discr_odd_prime [IsCyclotomicExtension {p} K L] [hp : Fact (p : ℕ).Prime] (hζ : IsPrimitiveRoot ζ p)
  ...
  · have h := even_sub_one_of_prime_ne_two hp.out hodd'
    rw [← mul_oneₓ 2, ← Nat.div_mul_div_comm (even_iff_two_dvd.1 h) (one_dvd _), Nat.div_oneₓ, mul_oneₓ, mul_comm,
      pow_mulₓ]
    congr 1
  ...
  · have H := congr_argₓ derivative (cyclotomic_prime_mul_X_sub_one K p)
    ...
    rw [MonoidHom.map_mul, hζ.sub_one_norm_prime hirr hodd, MonoidHom.map_mul, MonoidHom.map_pow,
      hζ.norm_eq_one hodd hirr, one_pow, mul_oneₓ, ← map_nat_cast (algebraMap K L), norm_algebra_map, finrank _ hirr,
      totient_prime hp.out, ← succ_pred_eq_of_pos hpos, pow_succₓ, mul_comm _ (p : K), coe_coe, ←
      hζ.minpoly_eq_cyclotomic_of_irreducible hirr] at H
    simpa [(mul_right_inj' <| NeZero.ne ↑↑p).1 H]
    infer_instance
